...
uint32_t TimerA_Capture_PulseWidth(GPTM_Type timer_N, DIO_AllPINS_TYPE ccp_pin)
{
    ...
    switch (timer_N)
    {
        ...
        case TIMER5:
            while(1)
            {
                INSERT_BIT(TIMER5_ICR_R, 2UL, 1UL);             /* clear timerA capture flag */
...
//                INSERT_BIT(WTIMER0_ICR_R, 2UL, 1UL);             /* clear timerA capture flag */
//                while((READ_BIT(WTIMER0_RIS_R, 2)) == 0);         /* wait till captured */
//                if(DIO_ReadPin(ccp_pin))                        /*check if rising edge occurs */
//                {
//                    risingEdge = WTIMER0_TAR_R;     /* save the timestamp */
//                    /* detect falling edge */
//                    INSERT_BIT(WTIMER0_ICR_R, 2UL, 1UL);             /* clear timer0A capture flag */
//                    while((WTIMER0_RIS_R & (1<<2)) == 0);            /* wait till captured */
//                    fallingEdge = WTIMER0_TAR_R;    /* save the timestamp */
//                    return (fallingEdge - risingEdge); /* return the time difference */
//                }
//                return 0; /* return 0 if no signal */
...
//            INSERT_BITS(GPIO_PORTA_PCTL_R, 0x07UL, pin_num*4UL, 4UL);   /* Data sheet page 688 */
            GPIO_PORTA_PCTL_R |= (0x07<<pin_num*4UL);
            DIO_InitPin(pin, OUTPUT);
            break;
        case PORTB:
            DIO_PORT_Init(PORTB);
            INSERT_BIT(GPIO_PORTB_AFSEL_R, pin_num, 1UL);
//            INSERT_BITS(GPIO_PORTB_PCTL_R, 0x07UL, pin_num*4UL, 4UL);   /* Data sheet page 688 */
            GPIO_PORTB_PCTL_R |= (0x07<<pin_num*4UL);
            DIO_InitPin(pin, OUTPUT);
            break;
        case PORTC:
            DIO_PORT_Init(PORTC);
            INSERT_BIT(GPIO_PORTC_AFSEL_R, pin_num, 1UL);
            GPIO_PORTC_PCTL_R |= (0x07<<pin_num*4UL);
            DIO_InitPin(pin, OUTPUT);
            break;
        case PORTD:
            DIO_PORT_Init(PORTD);
            INSERT_BIT(GPIO_PORTD_AFSEL_R, pin_num, 1UL);
            GPIO_PORTD_PCTL_R |= (0x07<<pin_num*4UL);
            DIO_InitPin(pin, OUTPUT);
            break;
        case PORTE:
            DIO_PORT_Init(PORTE);
            INSERT_BIT(GPIO_PORTE_AFSEL_R, pin_num, 1UL);
            GPIO_PORTE_PCTL_R |= (0x07<<pin_num*4UL);
            DIO_InitPin(pin, OUTPUT);
            break;
        case PORTF:
            DIO_PORT_Init(PORTF);
            INSERT_BIT(GPIO_PORTF_AFSEL_R, pin_num, 1UL);
            GPIO_PORTF_PCTL_R |= (0x07<<pin_num*4UL);
            DIO_InitPin(pin, OUTPUT);
            break;
        default:
            break;
